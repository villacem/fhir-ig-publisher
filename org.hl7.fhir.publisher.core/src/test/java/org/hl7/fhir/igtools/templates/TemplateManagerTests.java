...
public class TemplateManagerTests {
	...
	public void testLoadCustomTemplate() throws IOException {
		FHIRException thrown = Assertions.assertThrows(FHIRException.class, () ->  {IWorkerContext.ILoggingService loggingService  = mock(IWorkerContext.ILoggingService.class);
		TemplateManager templateManager = new TemplateManager(packageCacheManager, loggingService);
		...
		template.onLoadEvent(getImplementationGuide(), new HashMap<>());});
		assertTrue(thrown.getMessage().contains("ant.xml"));
		assertTrue(thrown.getMessage().contains("not trusted"));
	}
	@Nonnull
